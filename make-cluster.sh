...
initCluster() {
    ...
    verboseRun /data/server/redis-3.2.1/src/redis-trib.rb create --replicas $REPLICAS $TRIBARGS
...
stopCluster() {
    for PORT in `seq $START_PORT $END_PORT`; do
        verboseRun /data/server/redis-3.2.1/src/redis-cli -p $PORT SHUTDOWN NOSAVE > /dev/null 2>&1
    done
}

# Make sure we have redis-server and /data/server/redis-3.2.1/src/redis-trib.rb on the path
checkExe redis-server
checkExe /data/server/redis-3.2.1/src/redis-trib.rb
