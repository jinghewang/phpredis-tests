...
require_once(dirname($_SERVER['PHP_SELF'])."/RedisTest2.php");
 ...
 */
class Redis_Cluster_Test extends Redis_Test2 {
    ...
    public function __construct() {
        $str_nodemap_file = '/data/server/phpredis' . '/nodes/nodemap';
    ...
    public function testPing2222() {
        $obj = new RedisCluster(NULL, self::$_arr_node_map);
        var_dump($obj);
        $this->redis->set('xo','wjh');
        $result = $this->redis->get('xo');
        var_dump($result);
        $this->assertTrue($obj);
    }

    public function testPing3333() {
        $obj = new RedisCluster(NULL, self::$_arr_node_map);
        $result = $this->redis->get('xo');
        var_dump($result);
        $this->assertTrue($obj);
